...
#define OUTPUT_GPIO_YELLOW  CONFIG_EXAMPLE_OUTPUT_GPIO_YELLOW
...
esp_err_t app_driver_set_gpio(const char *name, bool state)
{
    if (strcmp(name, "Red") == 0) {
        ...
        gpio_set_level(OUTPUT_GPIO_YELLOW, state);
    } else {
        return ESP_FAIL;
    }
    return ESP_OK;
...
void app_driver_init()
{
    ...
    uint64_t pin_mask = (((uint64_t)1 << OUTPUT_GPIO_RED ) | ((uint64_t)1 << OUTPUT_GPIO_GREEN ) | ((uint64_t)1 << OUTPUT_GPIO_BLUE ) | ((uint64_t)1 << OUTPUT_GPIO_YELLOW ));
    ...
    gpio_set_level(OUTPUT_GPIO_YELLOW, false);
}
