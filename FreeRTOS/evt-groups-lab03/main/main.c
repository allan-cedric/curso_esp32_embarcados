...
#define LED2_PIN 14          //GPIO14
#define LED3_PIN 33          //GPIO33
...
void app_main(void)
{
    ...
    // Configure LED
    io_conf.pin_bit_mask = (1ULL<<LED1_PIN)|(1ULL<<LED2_PIN)|(1ULL<<LED3_PIN) ; // Bitmask to select the GPIOs you want to configure
...
void vTask1( void * parameter )
{
    ...
    while(1)
    {
        ...
        int read_button = gpio_get_level(BUTTON1_PIN);
        if(read_button == 0)
        {
          debouncing++;
          if(debouncing >= BUTTON_DEBOUNCING)
          {
            ESP_LOGI(TAG,"Task 1 configurou o evento:%d", TASK_1_BIT);
