...
#define EXAMPLE_ESP_WIFI_PASS      CONFIG_ESP_WIFI_PASSWORD     //wifi password
#define EXAMPLE_ESP_MAXIMUM_RETRY  CONFIG_ESP_MAXIMUM_RETRY     //wifi maximum retry
                                ...
                                int32_t event_id, void* event_data)
{
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {                 //if wifi event and wifi event station start
        ...
        ESP_LOGW(TAG, "DISCONNECTED %d: %s", wifi_event_sta_disconnected->reason,       //log wifi disconnected
                 get_wifi_disconnection_string(wifi_event_sta_disconnected->reason));   //get wifi disconnection string

        if (wifi_event_sta_disconnected->reason == WIFI_REASON_NO_AP_FOUND ||           //if wifi reason no ap found or
            wifi_event_sta_disconnected->reason == WIFI_REASON_ASSOC_LEAVE ||           //if wifi reason assoc leave or
            wifi_event_sta_disconnected->reason == WIFI_REASON_AUTH_EXPIRE)             //if wifi reason auth expire
        {
...
esp_err_t wifi_init_sta(void)
{
    ...
    EventBits_t bits = xEventGroupWaitBits(s_wifi_event_group,                      //wait for event group bits
                                            WIFI_CONNECTED_BIT | WIFI_FAIL_BIT,     //wifi connected bit or wifi fail bit
                                            pdFALSE,                                //no clear bits on exit
                                            pdFALSE,                                //no wait for all bits
                                            portMAX_DELAY);                         //wait until bits are set
