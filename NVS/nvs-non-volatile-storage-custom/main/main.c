...
void app_main(void)
{
    ...
    nvs_get_stats("nvs_ext", &nvs_stats);
    ESP_LOGI(TAG, "used: %d, free: %d, total: %d, namespace_count: %d", 
    ...
    err = nvs_open_from_partition("nvs_ext", "storage", NVS_READWRITE, &nvs_handle);
    if (err != ESP_OK)
    {
